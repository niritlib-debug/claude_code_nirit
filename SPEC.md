...
**Status:** v1 built; data read from a CSV file (mock data) or an imported CSV · spec last updated 2026-09-23
**Language:** English
...
- Saving an imported file (it lives only until the page is refreshed) or sending it to a server
...
### 4.6 Import your own CSV

Purpose: let the user look at different data (for example a real HR export) without touching the code or the repo.

- **Import CSV** (big button, pink, with an import icon) opens the computer's file picker, limited to `.csv` files.
...
- **Use sample data** (shown only while an imported file is on screen) goes back to the built-in sample.
- **Which week is "this week":** an imported file uses **today's date**. If the file has no hires in the 12 weeks before today (an old file), the dashboard shows the 12 weeks up to the file's latest start date and says so in a note under the data-source line.
...
- **Current week:** the week that contains the "as of" date. For the built-in sample file this date is frozen at **2026-09-19** (constant `AS_OF` at the top of `app.js`) so the dashboard looks the same whenever it is opened. An **imported** file always uses today's date (with the old-file fallback in §4.6). If the built-in file is ever replaced with real data, set `AS_OF` to `null`.
- The built-in sample and an imported file go through exactly the same reading and checks; only the "as of" date differs.
...
- Reading the built-in CSV needs the page to be served over http(s) (GitHub Pages or `python3 -m http.server`). A page opened straight from disk cannot read it, so it shows a pink message that says what to do, and mentions the Import CSV button.
- **Privacy:** a real employee CSV must **not** be committed to a public GitHub repo. Keep it out of git (`.gitignore`) or use a private repo. The CSV in this repo is fictional. An imported file is different: it stays in the browser and is never sent or saved (§4.6).
...
- [x] Import CSV: a valid file replaces the data everywhere (KPIs, chart, departments, Recent joiners, date range) and keeps the chosen range; "Use sample data" brings the sample back.
- [x] Import CSV: contractors are ignored, rows with a bad date are skipped and counted in the data-source line, an unknown department goes to Other.
...
- To look at real data, use **Import CSV** (§4.6): the file stays in the viewer's browser and never reaches GitHub. If Urban's rules require the tool itself to be internal, host it privately instead of on public GitHub Pages.

## 13. Next steps

1. Look at real data with **Import CSV** (§4.6) and keep the real file out of public GitHub (§12).
...
| 2026-09-19 | An Upload CSV button was added, then removed the same day: the request was for a **Download** button. Added **Download CSV**, which saves the mock data as `urban-employees-mock.csv`. |
